...
public class ChatRowArticle extends ChatRow {
	...
	private void addViews(ArticlesInfo msgArticles) {
		...
		if (msgArticles.getArticles().size() == 1) {
			...
			Glide.with(getContext()).load(bean.getPicurl()).into((ImageView) view.findViewById(R.id.article_main_pic));
	...
	private View createArticles(final ArticlesInfo.ArticleItem bean, Boolean isFirst) {
		...
		if (isFirst) {
			...
			Glide.with(getContext()).load(bean.getPicurl()).into(mainImage);
			...
			Glide.with(getContext()).load(bean.getPicurl()).into(imageView);
